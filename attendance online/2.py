...
class ThreadClass(QThread):
    val = pyqtSignal(tuple)
    def __init__(self):
        super(ThreadClass, self).__init__()
        self.isRunning = True
        ###########################database_connection####################
        self.host = 'sql5.freemysqlhosting.net'
        self.name = 'sql5460899'
        self.username = 'sql5460899'
        self.password = 'ZdFZGaRnU1'
        self.port = 3306
        ###############################################################################
    def run(self):
...
class mainApp(QMainWindow, FORM_CLASS):
    ...
    def controllers(self):
        ...
        self.pushButton_7.clicked.connect(self.exportToExcel)

        self.tableWidget_3.cellPressed.connect(lambda: self.finish_1())
    ...
    def login(self):
        ...
        try:
            db = mdb.connect(self.host , self.username, self.password,  self.name,self.port)
            cr = db.cursor()
            cr.execute('''SELECT * FROM users WHERE name='{}' AND password='{}' '''.format(user_name, user_password))
            data = cr.fetchall()
            db.commit()
            db.close()
    ...
    def delete(self):
        ...
        try:
            ...
            db = mdb.connect(self.host , self.username, self.password,  self.name, self.port)
            cr = db.cursor()
            cr.execute('''DELETE FROM workers WHERE name='{}' '''.format(item))
            db.commit()
            db.close()
            self.fill_table()
        except:
    ...
    def adddata_to_database(self):
        ...
        db = mdb.connect(self.host, self.username, self.password, self.name, self.port)
        cr = db.cursor()
        cr.execute('''INSERT INTO workers(name, job, section, phone, salary, address) VALUES('{}','{}','{}','{}','{}','{}')'''.format(Enter_Name, Enter_Job, Enter_Section, Enter_Phone, Enter_Salary, Enter_Address))
        db.commit()
        db.close()
        self.fill_table()
        self.widg.close()

    def fill_table_each_second(self):
        # thread_class object
        self.thread_1 = ThreadClass()
        self.thread_1.val.connect(self.fill)
        self.thread_1.start()
    ...
    def exportToExcel(self):
        ...
        for row in range(self.tableWidget_3.rowCount()):
            ...
            for col in range(self.tableWidget_3.columnCount()):
                record.append(self.tableWidget_3.item(row, col).text())
            rows.append(record)
